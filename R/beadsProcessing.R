get.calibration <- function(dir_path, bead_frame, flu_channels, cal_bead_peaks){
  ...
  fluorescence_cluster <- flowClust::flowClust(singlet_beads,
                                               varNames = flu_channels,
                                               K = 8);

  flowClust::plot(x=fluorescence_cluster, data=singlet_beads, subset=c(3,4))
  ...
  for (i in 1:length(flu_channels)){
    hist_plt <- ggplot2::ggplot()+
      ggplot2::geom_histogram(data=as.data.frame(log10_bead_frame@exprs),
                              ggplot2::aes(log10_bead_frame[,flu_channels[i]]@exprs, y=..count..),
                              bins=200, alpha=0.25)+
      ggplot2::geom_histogram(data=as.data.frame(singlet_beads@exprs),
                              ggplot2::aes(singlet_beads[,flu_channels[i]]@exprs, y=..count..),
                              bins=200, alpha=0.75)+
      ggplot2::geom_vline(xintercept = flowClust::getEstimates(fluorescence_cluster, singlet_beads)$locationsC[,i],
                          linetype=2, size=0.2)+
      ggplot2::scale_x_continuous(paste("log10", flu_channels[i], "(a.u.)"))+
    ...
    for(j in 1:length(cal_bead_peaks)){
      if(cal_bead_peaks[[j]]$channel == flu_channels[i]){
    ...
    calibration_parameters <- rbind(calibration_parameters,
                                    data.frame(flu=flu_channels[i], m=coef(model)["m"], b=coef(model)["b"]))
    ...
    cal_plt <- ggplot2::ggplot()+
      ...
      ggplot2::scale_x_continuous(name = paste("log10", flu_channels[i], "(a.u.)"), limits = c(0,6.5))+
      ggplot2::scale_y_continuous(name = paste("log10", flu_channels[i], "(MEF)"), limits = c(0,6.5))+
      ggplot2::theme_bw(base_size = 8)

    plt <- gridExtra::arrangeGrob(grobs = list(hist_plt, cal_plt), ncol = 2)
    ggplot2::ggsave(plot = plt, filename = paste(dir_path,
                                                 "_trimmed/", flu_channels[i], "_calibration.pdf",
